...
template <class TYPE>
void _rmatrix<TYPE>::PushCol(const _rmatrix& matrix)
{
	if(mem_size==0)
	{
		*this=matrix;
	...
	{
		assert( matrix.RowSize() == RowSize() && "_rmatrix::PushCol()  -> Row dimensions do not agree");
		_rmatrix<TYPE> temp = *this;
		col+=matrix.ColSize();
		int n = row*col;
		delete [] element;
		element = new TYPE[mem_size = n <<1];
		int pastSize=temp.RowSize() *temp.ColSize();
		for ( int i = 0 ; i < pastSize ; i++)
			element[i]=temp[i];
		for ( int i = 0 ; i < matrix.RowSize() * matrix.ColSize() ; i++)
            element[pastSize+i]= matrix[i];
	}
}

template <class TYPE>
void _rmatrix<TYPE>::PushRow(const _rmatrix& matrix)
{
	if(mem_size==0)
	{
		*this=matrix;
	...
	{
		assert( matrix.ColSize() ==ColSize() && "_rmatrix::PushRow() -> Col dimensions do not agree");
		_rmatrix<TYPE> temp = *this;
		row +=matrix.RowSize();
		int n = row*col;
		delete [] element;
		element = new TYPE[mem_size = n <<1];
		int sumSize = temp.RowSize() + matrix.RowSize();

		for ( int i = 0 ; i < ColSize() ; i++)
		{
			for(int j = 0 ; j < temp.RowSize() ; j++)
				element[i*sumSize + j] = temp[i*temp.RowSize()+j];
			for(int j = 0 ; j < matrix.RowSize() ; j++)
				element[i*sumSize + temp.RowSize()  + j] = matrix[i*matrix.RowSize()+j];
		}
	}
...
template <class TYPE>
void _rmatrix<TYPE>::DelRow(int r)
{
	assert( r>=0 && r< row  && "_rmatrix::DelRow() -> index exceeds matrix dimensions");
	...
	for( int i = 0 ; i < col ; i++)
	{
		...
		{
			if(j!=r)
				element[i*row+j] = temp[i*(row+1)+j+del_j];
			else // j == r
			{
				del_j=1;
				element[i*row+j] = temp[i*(row+1)+j+del_j];
...
template <class TYPE>
void _rmatrix<TYPE>::DelCol(int c)
{
	assert( c>=0 && c< col  && "_rmatrix::DelCol() -> index exceeds matrix dimensions");
	...
	for ( int i = 0 ; i < c ; i++ )
		for ( int j = 0 ; j < row ; j++ )
			element[i*row+j] = temp[i*row+j];
	for ( int i = c ; i < col ; i++ )
		for ( int j = 0 ; j < row ; j++ )
			element[i*row+j] = temp[(i+1)*row+j];
}
